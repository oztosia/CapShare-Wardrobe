package com.example.mypersonalwardrobe.source
...
open class FirebaseGenericRepo {
  ...
  suspend fun getWithCoroutines(path: String, documentPath: String, value: String): String {
   ...
   suspend fun deleteFromStorage(storageDocId: String, storagePath: String) {
       Log.w(TAG, "To delete: " + storagePath + storageDocId )
       try {
           runBlocking { firebaseStorage.child(storagePath).child(storageDocId).delete().await()
               Log.w(TAG, "Deleted: " + storagePath + storageDocId )
           }
       }
       catch (e:Exception){
